<table class="results">
    ...
    <tbody>
        @foreach ($manuscripts as $manuscript)
            <tr>
                <td><span class="bg-gray-600 text-white rounded px-2 py-1 font-bold">{{ $manuscript->getMeta('temporal') }}</span></td>
                <td>
                    @if ($manuscript->folios->first()->contentImage)
                        <img src="{{ route('iiif.image.requests', [$manuscript->folios->first()->contentImage->identifier, 'full', '100,', '0', 'default', 'jpg']) }}"
                            class="max-w-md border border-gray-300 bg-white p-1 rounded"
                            loading="lazy">
                    @endif
                </td>
                <td>
                    <a class="text-blue-800" href="{{ route('manuscript.show', $manuscript->name) }}">{{ $manuscript->name }}</a>
